...
public class BlazeAI : MonoBehaviour
{
    ...
    public bool useNaturalVelocity = true;
    [Tooltip("Set manually the velocity of the rigidbodies in ragdoll. You can change this according to the type of death, for example: increase the Y axis when AI is killed by an explosion to have it fly into the air.")]
    ...
    void EnableRagdoll()
    {
        ...
        foreach (Collider c in ragdollColls)
        {
            ...
            {
                c.attachedRigidbody.velocity = ragdollVelocity;
            }
    ...
    public void ChangeState(string state)
    {
        ...
        // reset & disable ragdoll if being used
        if (useRagdoll)
        {
            DisableRagdoll();
        }
    ...
    public void Hit(GameObject enemy = null, bool callOthers = false, int hitKind = 0)
    {
        ...
        if (!canBeAttack)
        {
            float hitWeight = Mathf.Pow(hitKind, 0.5f);
            this.anim.SetLayerWeight(1, 0.2f * hitWeight);
